...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(
        ...
        num_input_features: int,
        bn_size: int,
        growth_rate: int,       #the channel number of conv layer
        drop_rate: float,
        memory_efficient: bool = False
    ) -> None:
...
class DenseNet(nn.Module):
    ...
    def dropout_layer(self, x):
        ...
        #dim=2：在时间域上的均值。因为在之前模型的运算中，数据被转换为三维。[batch_size, data, time_step]
        p = (d - d.min()) / (d.max() - d.min())
        p = 1 - (1 - p) * self.drop_rate / (1 - p.mean())
        p = torch.clamp(p, min=0., max=1.)
        #限制p的大小在0-1区间，小于0的替换为0，大于1的替换为1
        d = torch.bernoulli(p)
        #根据 生成0/1分布
        d = d.div(torch.where(p > 0, p, torch.tensor(1.).to(x.device)))
        #torch.where(condition, a, b):满足condition条件，选择a,否则，选择b
        #torch.div:做除法
...
def densenet102(args=None, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_.
    """
    return _densenet(args, 'densenet102', growth_rate=32, block_config=(12, 12, 24), num_init_features=256,
