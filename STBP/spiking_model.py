...
class ActFun(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return input.gt(thresh).float()     #greater than
    ...
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone()
        temp1 = abs(input - thresh)
        temp2 = 2 - temp1 > 0
        temp = (0.5 - 0.25 * temp1) * temp2
        return grad_input * temp.float()'''
    ...
    def forward(self, input, time_window = 20):
       ...
       #h3_mem = h3_spike = h3_sumspike = torch.zeros(batch_size, cfg_fc[2], device=device)
        ...
        for step in range(time_window): # simulation time steps
            ...
            c1_mem, c1_spike = mem_update(self.conv1, x.float(), c1_mem, c1_spike)      #[100, 12, 28, 28]

            x = F.avg_pool2d(c1_spike, 2)                                               #[100, 12, 14, 14]

            c2_mem, c2_spike = mem_update(self.conv2,x, c2_mem,c2_spike)                #[100, 64, 14, 14]

            x = F.avg_pool2d(c2_spike, 2)                                               #[100, 64, 7, 7]
            x = x.view(batch_size, -1)                                                  #[100, 64*7*7]
